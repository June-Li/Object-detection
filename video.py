...
def main_fun():
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            with torch.no_grad():
                output = model(Variable(img, volatile=True), CUDA)  # 这个是模型的预测输出，还是毛坯型
            output = write_results(output, confidence, num_classes, nms_conf=nms_thesh)  # 对毛坯型打磨后生成output，它是D×8
            ...
            im_dim = im_dim.repeat(output.size(0), 1)  # [[x,y,x,y],[x,y,x,y]……]，x，y是原始图像的宽和高
            ...
            list(map(lambda x: write(x, frame, colors, classes), output))
